...
class PredictionPipeline:
    ...
    def __init__(self, use_downsampled: bool = False):
        ...
        # 提交文件保存路径
        project_root = self.processor.get_project_root() if hasattr(self.processor, 'get_project_root') else self.processor.PROJECT_ROOT
    ...
    def load_and_preprocess_data(self) -> tuple:
        ...
        # 特征缩放
        X_train_scaled, X_val_scaled = self.processor.scale_features(X_train, X_val, scaler_type='robust')
    ...
    def train_gradient_boosting_models(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        if tune_hyperparams:
            xgb_results = xgb_trainer.hyperparameter_tuning(X_train, y_train, X_val, y_val, n_iter=20)
        else:
        ...
        if tune_hyperparams:
            lgb_results = lgb_trainer.hyperparameter_tuning(X_train, y_train, X_val, y_val, n_iter=20)
        else:
    ...
    def create_ensemble(self, X_val: pd.DataFrame, y_val: pd.Series,
        ...
        print(f"集成模型验证集皮尔逊相关系数: {ensemble_metrics['pearson_correlation']:.6f}")
        
        return ensemble_metrics
    
    def generate_predictions(self, X_test: pd.DataFrame, test_ids: np.ndarray,
    ...
    def run_full_pipeline(self, tune_hyperparams: bool = False,
                         use_lstm: bool = True, lstm_window_size: int = 10,
                         ensemble_method: str = 'weighted_average') -> str:
        """
        运行完整的预测管道
        ...
        # 1. 数据预处理
        X_train, X_val, X_test, y_train, y_val, test_ids = self.load_and_preprocess_data()
...
def medium_scale_training(downsample_ratio: float = 0.3) -> str:
    ...
    if len(train_df) > 100000:  # 如果数据太大，重新采样
        ...
        downsampled_file = os.path.join(processor.processed_data_dir, f"drw-crypto-train_medium_{int(downsample_ratio*100)}.parquet")
    ...
    X_train, X_val, X_test, y_train, y_val, test_ids = pipeline.load_and_preprocess_data()
