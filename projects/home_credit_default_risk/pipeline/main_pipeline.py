...
class HomeCreditPipeline(BasePipeline):
        ...
        try:
            ...
            self.pipeline_results = {
                ...
                'pipeline_metadata': {
                    ...
                    'data_shape': self.features_df.shape if self.features_df is not None else (0, 0),
                    'selected_features_count': self.selected_features_df.shape[1] if self.selected_features_df is not None else 0
                }
            }
    ...
    def _clean_data(self):
        ...
        self._log_info(f"数据清洗完成，处理了 {cleaning_stats.get('total_datasets', 0)} 个数据集")
    ...
    def _build_features(self):
        ...
        # 获取目标变量
        if 'application_train' in self.cleaned_data and 'TARGET' in self.cleaned_data['application_train'].columns:
            self.target = self.cleaned_data['application_train']['TARGET']
        
        # 构建特征
        self.features_df = self.feature_builder.fit_transform(self.cleaned_data, target=self.target)
        
        # 记录特征统计
        feature_stats = self.feature_builder.get_feature_stats()
        self._log_info(f"特征工程完成，生成了 {feature_stats.get('final_features', 0)} 个特征")
    ...
    def _select_features(self):
        ...
        if self.target is not None:
            ...
            self.selected_features_df = self.feature_selector.fit_transform(features_aligned, target_aligned)
        else:
            self.selected_features_df = self.feature_selector.fit_transform(self.features_df)
        
        # 记录选择统计
        selection_stats = self.feature_selector.get_selection_stats()
        self._log_info(f"特征选择完成，选择了 {selection_stats.get('selected_features', 0)} 个特征")
    ...
    def _train_advanced_models(self) -> Dict:
        ...
        if self.target is not None:
            ...
            self._log_info(f"高级模型训练完成，最佳模型: {best_model_name} (AUC: {best_score:.4f})")
    ...
    def _train_ensemble_models(self) -> Dict:
        ...
        if self.target is not None and self.model_trainer is not None:
            ...
            ensemble_results = self.ensemble_model.train(
                X_train, y_train, 
                trained_models=self.model_trainer.trained_models
            )
            
            best_ensemble_name, _, best_score = self.ensemble_model.get_best_ensemble()
            self._log_info(f"集成模型训练完成，最佳集成: {best_ensemble_name} (AUC: {best_score:.4f})")
    ...
    def _final_evaluation(self) -> Dict:
        ...
        if self.target is not None:
            ...
            if self.ensemble_model and self.ensemble_model.is_trained:
                ...
                final_results['best_model_name'] = self.ensemble_model.best_ensemble_name
                ...
                final_results['best_model_name'] = self.baseline_model.best_baseline_model
                final_results['evaluation'] = eval_results
    ...
    def is_trained(self) -> bool:
        """检查是否已训练"""
        return (
            (self.baseline_model and self.baseline_model.is_trained) or
            (self.model_trainer and self.model_trainer.is_trained) or
            (self.ensemble_model and self.ensemble_model.is_trained)
        )
    ...
    def get_pipeline_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'advanced_models_trained': self.model_trainer and self.model_trainer.is_trained,
